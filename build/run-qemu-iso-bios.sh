#!/bin/bash
# Boot a kit ISO under QEMU with LEGACY BIOS (SeaBIOS, no OVMF) and screendump.
set -u
ISO=$1; TAG=$2; DELAY=${3:-8}; TOTAL=${4:-50}
SOCK=/tmp/qmp-$TAG.sock; rm -f "$SOCK" "scr_$TAG.ppm" "serial_$TAG.log"
